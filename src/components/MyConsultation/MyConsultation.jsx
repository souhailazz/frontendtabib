...
const MyConsultation = () => {
  ...
  const fetchOrdonnance = async (consultationId) => {
    ...
    try {
      const response = await axios.get(`https://api.tabib.life/api/ordonnances/consultation/${consultationId}`);
  ...
  const fetchConsultations = useCallback(async () => {
    ...
    const url = userType === 'patient'
      ? `https://api.tabib.life/api/consultations/patient/${userId}`
      : `https://api.tabib.life/api/consultations/doctor/${userId}`;
  ...
  const fetchMessages = useCallback(() => {
    ...
    if (currentSelected) {
      ...
      if (patientId && doctorId) {
        axios
          .get(`https://api.tabib.life/api/messages/between?docteurId=${doctorId}&patientId=${patientId}`)
  ...
  const handleSendMessage = async () => {
    ...
    try {
      if (!consultation) {
        ...
        const res = await axios.post('https://api.tabib.life/api/consultations', newConsultation);
      ...
      if (!ordonnanceId) {
        ...
        try {
          ...
          if (ordonnance && ordonnance.id) {
            ...
            const ordonnanceResponse = await axios.post(`https://api.tabib.life/api/ordonnances`, ordonnancePayload);
      ...
      const res = await axios.post('https://api.tabib.life/api/messages', dto);
  ...
  const handleEditOrdonnance = async (ordonnanceId, content) => {
    try {
      ...
      const response = await axios.put(`https://api.tabib.life/api/ordonnances/${ordonnanceId}`, {
        contenu: content,
        status: 'active'
      });
  ...
  const handleUpdateConsultationStatus = async (consultationId, newStatus) => {
    try {
      const response = await axios.put(`https://api.tabib.life/api/consultations/${consultationId}/status?status=${newStatus}`);
      if (response.data) {
