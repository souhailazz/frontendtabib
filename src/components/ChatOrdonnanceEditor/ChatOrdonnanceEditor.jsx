...
const ChatOrdonnanceEditor = ({ consultationId, onClose }) => {
  ...
  useEffect(() => {
    const loadOrdonnance = async () => {
      try {
        const res = await axios.get(`https://api.tabib.life/api/ordonnances/consultation/${consultationId}`);
  ...
  const updateOrdonnance = async () => {
    try {
      await axios.put(`https://api.tabib.life/api/ordonnances/${ordonnance.id}`, {
        ...ordonnance,
        contenu,
      });
      onClose();
