...
const CreditCardForm = ({ amount, onPaymentSuccess, onPaymentError, bookingData }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch("https://api.tabib.life/api/payments/create-payment-intent", {
        method: "POST",
        headers: {
      ...
      if (paymentIntent.status === "succeeded") {
        ...
        const confirmResponse = await fetch(`https://api.tabib.life/api/payments/confirm-payment?paymentIntentId=${paymentIntent.id}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
...
const MobileMoneyForm = ({ amount, onPaymentSuccess, onPaymentError, bookingData }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch("https://api.tabib.life/api/payments/mobile-money", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
const PayPalForm = ({ amount, onPaymentSuccess, onPaymentError, bookingData }) => {
  ...
  const handlePayPalPayment = async () => {
    ...
    try {
      ...
      const response = await fetch("https://api.tabib.life/api/payments/paypal", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
